...
async def handle_tsd_scan(request):
    ...
    logger.info(f"Получен и расшифрован JSON от ТСД: {data}")  
    ...
    # Обрабатываем результат: если вернулся кортеж (model, short_name, quantity) — лог и ответ ТСД с ними.
    if isinstance(db_operation_res, tuple) and len(db_operation_res) == 3:
        model_res, short_name_res, new_qty = db_operation_res
        logger.info(f"Обновление БД: Штрих-код={model_res}, Наименование={short_name_res}, Новое количество={new_qty}")
    ...
    if isinstance(db_operation_res, str):
        ...
        if db_operation_res.startswith("NO_STOCK:"):
            ...
            logger.info(f"БД не обновлена, нет на складе: {barcode_no_stock}")

            response_text = f"Нет на складе: {barcode_no_stock}"
            for user_id in user_ids:
                await bot.send_message(chat_id=user_id, text=f"Штрих-код: {barcode}\nДействие: {action}\n{response_text}")

            # 409 Кофликт с состоянием базы
            encrypted_response = encrypt_data(response_text)
            return web.Response(text=encrypted_response, status=409)

        # Любой другой вариант  — возвращаем 400, вряд ли произойдет, но мало ли, логируем и отправляем в тг
        logger.warning(f"Неожиданный ответ от БД: {db_operation_res}")
        response_text = "Неожиданный ответ от БД"
        await bot.send_message(539356755, f"Штрих-код: {barcode}\nДействие: {action}\n{response_text}")
        encrypted_response = encrypt_data(response_text)
